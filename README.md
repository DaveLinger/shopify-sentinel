...
| Back in stock (relist) | Product appears that wasn't in the previous fetch, is in stock, and was created more than 24 hours ago — i.e. a listing the store pulled and put back. Posted as `Back in Stock`, not as a new product |
...
| Back in stock | A tracked product's availability flips false → true after being out of stock for at least 20 minutes (flap damping) — detected from the same poll data, no extra requests. Also covers delist/relist stores: a product that vanished from the feed while out of stock and later reappears counts as out of stock for the whole gap. A product that vanishes and returns *without* any stock change is treated as a truncated fetch and stays quiet, unless it was gone more than 6 hours |

Product removals are detected and logged silently (no Discord alert). The entry is *tombstoned* rather than deleted, because some stores unpublish a product when it sells out instead of flagging it out of stock — it disappears from `/products.json` entirely. Deleting outright meant the eventual re-publish arrived as an ID the watcher had never seen, carrying its original `created_at` (often months old), so it failed the 24-hour new-product gate and never hit the availability transition either — no alert of any kind. Tombstones expire after 60 days; a product that returns after that — or one delisted before tombstoning existed — arrives as an unknown ID and is caught by the relist rule above instead. Large paginated catalogs drop products from the feed transiently, so a reappearance only counts as a restock if the product was out of stock when it went, or was gone more than 6 hours — otherwise nothing actually changed and no alert is sent.

Because an unknown ID now alerts regardless of its age, a genuine flood of them is capped rather than posted: more than 25 unknown products in a single poll (against a non-empty database) are recorded silently with a warning in the log. That covers widening a live deployment's collection or tag filter, switching it to the Storefront API's full catalog, and a truncated fetch. Alerts longer than Discord's message limit are split into multiple webhook posts, each repeating the header.
